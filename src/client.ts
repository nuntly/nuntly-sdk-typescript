...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['NUNTLY_LOG'] or 'warn' if it isn't set.
   */
...
export class Nuntly {
   ...
   */
  constructor({
    baseURL = readEnv('NUNTLY_BASE_URL'),
    ...
    this.timeout = options.timeout ?? Nuntly.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('NUNTLY_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as Nuntly, request, Page);
  }
