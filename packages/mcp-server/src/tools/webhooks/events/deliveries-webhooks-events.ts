// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

import { maybeFilter } from '@nuntly/sdk-mcp/filtering';
import { Metadata, asTextContentResult } from '@nuntly/sdk-mcp/tools/types';

import { Tool } from '@modelcontextprotocol/sdk/types.js';
import Nuntly from '@nuntly/sdk';

export const metadata: Metadata = {
  resource: 'webhooks.events',
  operation: 'read',
  tags: [],
  httpMethod: 'get',
  httpPath: '/webhooks/{id}/events/{event_id}/deliveries',
  operationId: 'list-webhook-event-deliveries',
};

export const tool: Tool = {
  name: 'deliveries_webhooks_events',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nReturn the delivery attempts for the given webhook event ID\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/event_deliveries_response',\n  $defs: {\n    event_deliveries_response: {\n      type: 'array',\n      items: {\n        type: 'object',\n        properties: {\n          id: {\n            type: 'string'\n          },\n          code: {\n            type: 'string'\n          },\n          delivered_at: {\n            type: 'string'\n          },\n          response: {\n            type: 'object',\n            additionalProperties: true\n          },\n          status: {\n            type: 'string',\n            enum: [              'pending',\n              'success',\n              'failed'\n            ]\n          }\n        },\n        required: [          'id',\n          'code',\n          'delivered_at',\n          'response',\n          'status'\n        ]\n      }\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      id: {
        type: 'string',
      },
      event_id: {
        type: 'string',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
        description:
          'A jq filter to apply to the response to include certain fields. Consult the output schema in the tool description to see the fields that are available.\n\nFor example: to include only the `name` field in every object of a results array, you can provide ".results[].name".\n\nFor more information, see the [jq documentation](https://jqlang.org/manual/).',
      },
    },
    required: ['id', 'event_id'],
  },
  annotations: {
    readOnlyHint: true,
  },
};

export const handler = async (client: Nuntly, args: Record<string, unknown> | undefined) => {
  const { event_id, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.webhooks.events.deliveries(event_id, body)),
  );
};

export default { metadata, tool, handler };
