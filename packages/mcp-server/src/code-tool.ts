...
export function codeTool(params: { blockedMethods: SdkMethod[] | undefined }): McpTool {
  ...
  const handler = async (client: Nuntly, args: any): Promise<ToolCallResult> => {
    ...
    const res = await fetch(codeModeEndpoint, {
      ...
      headers: {
        ...
        client_envs: JSON.stringify({
          NUNTLY_API_KEY: readEnv('NUNTLY_API_KEY') ?? client.apiKey ?? undefined,
          NUNTLY_BASE_URL: readEnv('NUNTLY_BASE_URL') ?? client.baseURL ?? undefined,
        }),
      },
      body: JSON.stringify({
        project_name: 'nuntly',
