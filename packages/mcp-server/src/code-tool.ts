...
import { ClientOptions } from '@nuntly/sdk';

const prompt = `Runs JavaScript code to interact with the Nuntly API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { NUNTLY_API_KEY: readEnv('NUNTLY_API_KEY') ?? client.apiKey ?? undefined, NUNTLY_BASE_URL: readEnv('NUNTLY_BASE_URL') ?? client.baseURL ?? undefined };
