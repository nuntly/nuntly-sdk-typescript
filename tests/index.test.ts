...
import { APIPromise } from '@nuntly/sdk/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NUNTLY_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Nuntly) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Nuntly({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Nuntly({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NUNTLY_LOG'] = 'debug';
      const client = new Nuntly({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NUNTLY_LOG'] = 'not a log level';
      const client = new Nuntly({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NUNTLY_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['NUNTLY_LOG'] = 'debug';
      const client = new Nuntly({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  test('normalized method', async () => {
    ...
    const client = new Nuntly({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Nuntly({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['NUNTLY_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Nuntly({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['NUNTLY_BASE_URL'] = 'https://example.com/from_env';
      const client = new Nuntly({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Nuntly({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['NUNTLY_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Nuntly({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Nuntly({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Nuntly({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new Nuntly({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Nuntly({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['NUNTLY_API_KEY'] = 'My API Key';
    const client = new Nuntly();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['NUNTLY_API_KEY'] = 'another My API Key';
    const client = new Nuntly({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Nuntly({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Nuntly({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Nuntly({ apiKey: 'My API Key', fetch: testFetch });
